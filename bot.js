...
client.on("message", async message => {
  ...
  if (lus) {
    const reklamengel = ["discord.app", "discord.gg", ".party", "", ".az", ".net", ".io", ".gg", ".me", "https", "http", ".com.tr", ".org", ".tr", ".gl", "glicht.me/", ".rf.gd", ".biz", "www.", "www"];
...
antispam(client, {
  ...
  rolİsimi: "771731634058952714 " // Spam Atan Kullanıcılar Verilecek Röl
});
...
client.on('guildMemberAdd', async (member) => {
 ...
 let channels = member.guild.channels.cache.find(c => c.name === '771731855182659595')
...
client.on("message", async message => {
  ...
  if (lus) {
    const reklamengel = ["amk", "oç", "orrrrrrrrrrr","siktir","siktirgit","siktir git"];
    if (reklamengel.some(word => message.content.toLowerCase().includes(word))) {
      try {
        if (!message.member.permissions.has('KICK_MEMBERS')) {
          message.delete();
...
client.on("message", async message => {
  ...
  if (a) {
      if (message.content.toLowerCase() === "sa","selam","selamun aleykum") {
